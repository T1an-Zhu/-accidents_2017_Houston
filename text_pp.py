...
def extract_detailed_content(lines):
    ...
    for line in lines:
        ...
        # 检查是否是海域部分
        if any(keyword in line for keyword in ['COASTAL WATERS', 'GALVESTON BAY', 'MATAGORDA BAY', 'WATERS FROM']):
        ...
        if '.DAY ONE...' in line:
            ...
            continue
        elif '.DAYS TWO THROUGH SEVEN...' in line:
            current_section = 'days_two_seven'
            continue
        elif '.SPOTTER INFORMATION STATEMENT...' in line:
    ...
    # 清理文本
    for key in ['day_one', 'days_two_seven', 'spotter_info']:
